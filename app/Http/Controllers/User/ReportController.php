...
class ReportController extends Controller {
	...
	public function account_statement(Request $request) {
		if ($request->isMethod('get')) {
			...
			DB::select("SELECT ((SELECT IFNULL(SUM(amount),0) FROM transactions WHERE dr_cr = 'cr' AND business_id = $business_id AND created_at < '$date1') - (SELECT IFNULL(SUM(amount),0) FROM transactions WHERE dr_cr = 'dr' AND business_id = $business_id AND created_at < '$date1')) into @openingBalance");
	...
	public function income_by_customer(Request $request) {
		if ($request->isMethod('get')) {
			...
			return view('backend.user.reports.income_by_customer', compact('page_title'));
		} else if ($request->isMethod('post')) {
			@ini_set('max_execution_time', 0);
			@set_time_limit(0);
